...
public partial class PlantillaGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            //jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceDosimetria.DeleteRecordQuestion + " " + item["Nombre"].Text, Resources.ResourceDosimetria.DeleteRecord);
            //imgb.OnClientClick = jCommand;
            //imgb.Visible = permiso.Crear;
        }
    }
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        Plantilla p = CntDosimetria.GetPlantilla(id, ctx);
                        CntDosimetria.CtxEliminar(p, ctx);
    ...
    {
        RadGrid1.DataSource = CntDosimetria.GetPlantillas(ctx);
        if (rebind)
            RadGrid1.Rebind();
    }
    #endregion

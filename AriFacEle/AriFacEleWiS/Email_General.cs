...
public class EmailGen
{
    ...
    public void mandarMail(Cliente cli, FacturaEntity ctx1)
    {
        ...
        */
        ...
        string emailUrl = ConfigurationSettings.AppSettings["url_email"];
        string tabla = "";
        ...
        foreach (Factura item in c.Facturas)
        {
            string link = emailUrl + "?Factura=" + item.Id_factura;
            if ((bool)item.Nueva)
        ...
        cuerpo = string.Format(planti.Contenido, c.Nombre, emailUrl + "?Factura", tabla);
    ...
    public bool MadarEmailGen(string subject, string body)
    {
        correo.From = new MailAddress(ConfigurationSettings.AppSettings["mail_address"]);
        correo.To.Add(new MailAddress(ConfigurationSettings.AppSettings["mail_to"]));
